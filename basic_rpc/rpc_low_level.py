...
from .rpc_serialization_functions import (int_to_le_bytes_4, int_to_le_bytes_2,
        int_to_le_bytes_1, int_from_le_bytes_4, int_from_le_bytes_2,
        serialize_str, deserialize_str, serialize_bool,
)

is_exception = lambda obj: isinstance(obj, type) and issubclass(obj, BaseException)
built_in_exceptions = {k:v for k,v in builtins.__dict__.items()
                            if is_exception(v)}
...
def unexpected_msg_error(expected:Enum, got:Enum):
    raise ProtocolError(
            f'Unexpected msg_type: {got.name}, should be: {expected.name}')
...
ClientMsgType, ClientMsgTypeBytes = gen_enum_and_bytes('ClientMsgType', [
    ('MSG_CLIENT_INIT',    0),
...
ServerMsgType, ServerMsgTypeBytes = gen_enum_and_bytes('ServerMsgType', [
    ('MSG_SERVER_INIT',      0),
