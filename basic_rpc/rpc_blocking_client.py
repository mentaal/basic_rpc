...
from .rpc_low_level import (ServerMsgTypeBytes,
    deserialize_exception_raise, REQ_HDR_PREFIX_SIZE, VERSION_BYTES,
    serialize_client_rpc_req, serialize_client_init,
    ProtocolError, DisconnectedError, unexpected_msg_error,
    ...
    def connect_init_wait(self, deadline:float, retry_interval_secs):
        while True:
            ...
            # also make the deadline more stringent (for whatever it's worth)
                ...
                raise ConnectionError('Timed out waiting for connection with server to be established (at the protocol layer, not socket layer)')
